...
export default function PeerComparison({ footprint }: PeerComparisonProps) {
  ...
  const benchmarkRegions: BenchmarkRegion[] = [
    {
      ...
      description:
        'High reliance on fossil-fuel transportation, high consumer goods throughput, and spacious home heating/cooling profiles.',
    ...
    },
    {
      ...
      description:
        'Increasing renewable integration in home power grids and density public transport systems, though still above stable limits.',
    ...
    },
    {
      ...
      description:
        'Global per-capita emission including developing areas. Reflects a mixture of urban sprawl and low-intensity rural living.',
    ...
    },
    {
      ...
      description:
        'The maximum individual annual threshold recommended by scientific panels to keep global rises below critical tipping bounds.',
  ...
  const getFeedbackMessage = () => {
    ...
    if (footprint.total <= GLOBAL_BENCHMARKS.worldAverage) {
      return {
    ...
    if (footprint.total <= GLOBAL_BENCHMARKS.euAverage) {
      return {
    ...
    if (footprint.total <= GLOBAL_BENCHMARKS.usAverage) {
      return {
  ...
  return (
    <div
      id="peer-comparison-module-card"
      className="p-6 bg-white rounded-3xl border border-slate-200/80 shadow-sm flex flex-col justify-between space-y-6"
    >
      ...
      <div className="space-y-5" id="comparative-gauges-container" role="group" aria-label="Benchmark comparison levels">
        <EmissionBar
          id="user-footprint-gauge-bar"
          label="Your Current Footprint"
          trailing={
            <span className="font-mono text-emerald-600">
              {footprint.total.toLocaleString()} kg CO2e/yr
            </span>
          }
          widthPercent={(footprint.total / maxBarValue) * 100}
                ...
                <EmissionBar
                  ...
                  trailing={
                    <span className="flex items-center gap-2 font-semibold">
                      <span className="font-mono text-slate-600">{region.value.toLocaleString()} kg</span>
                      {diff.isBetter ? (
                  ...
                  widthPercent={(region.value / maxBarValue) * 100}
                  dotClassName={region.dotClassName}
                  barClassName={region.barClassName}
      ...
      <div
        className={`p-4 rounded-2xl border ${feedback.color} transition-colors duration-300 flex gap-3 text-xs leading-normal items-start`}
      >
        <Info className="w-4 h-4 shrink-0 mt-0.5" aria-hidden="true" />
        <div className="space-y-1">
          <h4 className="font-extrabold tracking-tight underline decoration-dotted">{feedback.title}</h4>
          <p className="font-medium text-slate-600">{feedback.description}</p>
        </div>
      </div>
    </div>
  );
